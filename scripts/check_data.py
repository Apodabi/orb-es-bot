...
Verifies, per month:
  ...
  (c) no consecutive close-to-close jump > threshold; every jump is annotated
      (intra-RTH / roll-boundary / same-day globex / plain overnight) — bad
      roll stitches show up as large jumps at contract boundaries; only jumps
      inside RTH fail (globex news moves are real and warned, not failed)
  (d) no duplicate timestamps, no negative volume; zero-volume RTH bars fail
      on full sessions but only warn on holiday early-close sessions (quiet
      no-trade minutes are normal in holiday-thin trading)
...
def main() -> None:
    ...
    # zero-volume RTH bars: impossible on a full session; a quiet no-trade
    # minute on a holiday early-close session is normal
    zero_vol = rth[rth["volume"] == 0]
    for d in sorted(set(zero_vol.index.date)):
        n = int((zero_vol.index.date == d).sum())
        kind = sessions.get(d, {}).get("kind")
        if kind == "early_close":
            warnings.append(f"{n} zero-volume RTH bar(s) on early-close session {d} (holiday-thin trading)")
        else:
            failures.append(f"{n} zero-volume RTH bar(s) on {kind or 'unclassified'} session {d}")
    ...
    def in_rth(t: pd.Timestamp) -> bool:
    ...
    for t, p in jumps.items():
        ...
        kind = ("INTRA-RTH" if same_day and in_rth(prev_t) and in_rth(t)
                else "ROLL-BOUNDARY" if near_roll
                else "globex/same-day" if same_day
        ...
        if kind == "INTRA-RTH":
            failures.append(f"intra-RTH jump > {args.jump_pct}%: {line}")
        elif kind == "ROLL-BOUNDARY":
            failures.append(f"possible bad roll stitch: {line}")
        elif kind == "globex/same-day":
            warnings.append(f"globex move > {args.jump_pct}% (can be legitimate news): {line}")
        else:
    ...
    print(f"\njumps > {args.jump_pct}%: "
          f"{sum(1 for r in jump_rows if r[3] == 'INTRA-RTH')} intra-RTH, "
          f"{sum(1 for r in jump_rows if r[3] == 'ROLL-BOUNDARY')} roll-boundary, "
          f"{sum(1 for r in jump_rows if r[3] == 'globex/same-day')} globex, "
